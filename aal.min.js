!async function() {
    ...
    function isUserPremium() {
        ...
        return true;//!(!retPre || "1" != retPre) || (instance.platform == PLATFORM_CODES.STEAM || (!(instance.platform != PLATFORM_CODES.WINDOWS || !instance.wspo) || (!!checkAndRet(instance) || (!!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.getHostPremium && iosPremiumStatus && "1" == iosPremiumStatus.trim()) || (!(!window.Android || !window.Android.getHostPremium || window.Android.checkSKU || !Android.getHostPremium()) || !(!instance.loginCreds || !instance.profileData) && !(!instance.loginCreds || !instance.profileData || "1" != instance.profileData.pre))))))
    }
